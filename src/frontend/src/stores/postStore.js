  ...
  persist(
    (set, get) => ({
      ...
      search: async (query, username) => {
        set({ listLoading: true, listError: null });
        try {
          const encoded = encodeURIComponent(query.trim()).replace(
            /%20/g,
            "%20",
          );
          const { activeFilters } = usePostStore.getState();
      ...
      votePost: async (postId, voteState) => {
        ...
        const prev = get();
        const oldVoteInList =
          prev.posts.find((p) => p.id === postId)?.vote ?? 0;
        const delta = voteState - oldVoteInList;
        const oldCurrentVote =
          prev.current?.data?.id === postId
            ? (prev.current.data.vote ?? 0)
            : null;
        ...
        set((s) => ({
          posts: s.posts.map((p) =>
            p.id === postId
              ? { ...p, score: p.score + delta, vote: voteState }
              : p,
          ),
          current:
              ...
              oldCurrentVote !== null
                ? {
                    ...
                    data: {
                      ...s.current.data,
                      score:
                        s.current.data.score - (voteState - oldCurrentVote),
                      vote: oldCurrentVote,
                    },
                  }
                : s.current,
      ...
      importFromPr0gramm: async (
        { tags, flags = 1, maxPages = 5 },
        onProgress,
      ) => {
    ...
    {
      name: "wallium-post-filters",
      // Only persist the filter selection — not posts, pagination, or loading state.
      partialize: (state) => ({ activeFilters: state.activeFilters }),
    },
  ),
);
