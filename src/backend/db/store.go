...
const postWithVoteCols = `
    ...
    p.content_type, p.score, p.user_level,
    p.p_hash_0, p.p_hash_1, p.p_hash_2, p.p_hash_3,
    p.user_name, p.created_at, p.deleted_at, p.dirty, p.width, p.height,
...
// $1=userID (0 for anon), $2=beforeID, $3=userLevel, $4=limit.
func (s *Store) GetPostsOlderThan(ctx context.Context, beforeID, userID, userLevel, limit int32) ([]PostWithVote, error) {
	q := `SELECT` + postWithVoteCols + postWithVoteJoin + `
    WHERE p.id < $2 AND p.deleted_at IS NULL AND p.dirty = FALSE AND p.user_level <= $3
    ORDER BY p.id DESC
    LIMIT $4`
	return scanPostsWithVote(s.Pool, ctx, q, userID, beforeID, userLevel, limit)
}
...
// $1=userID (0 for anon), $2=afterID, $3=userLevel, $4=limit.
func (s *Store) GetPostsNewerThan(ctx context.Context, afterID, userID, userLevel, limit int32) ([]PostWithVote, error) {
	q := `SELECT` + postWithVoteCols + postWithVoteJoin + `
    WHERE p.id > $2 AND p.deleted_at IS NULL AND p.dirty = FALSE AND p.user_level <= $3
    ORDER BY p.id ASC
    LIMIT $4`
	posts, err := scanPostsWithVote(s.Pool, ctx, q, userID, afterID, userLevel, limit)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetPostsAround(ctx context.Context, postID, userID, userLevel, limit int32) (
	...
	newer, err = s.GetPostsNewerThan(ctx, postID, userID, userLevel, fetchLimit)
	if err != nil {
		return
	}
	...
	older, err = s.GetPostsOlderThan(ctx, postID, userID, userLevel, fetchLimit)
	if err != nil {
		return
	}
    ...
    WHERE p.id = $2 AND p.deleted_at IS NULL AND p.dirty = FALSE AND p.user_level <= $3`
	rows, qErr := s.Pool.Query(ctx, tq, userID, postID, userLevel)
	if qErr != nil {
		err = qErr
		return
	}
	...
	if rows.Next() {
		if scanErr := rows.Scan(
			...
			&target.ContentType, &target.Score, &target.UserLevel,
			&target.PHash0, &target.PHash1, &target.PHash2, &target.PHash3,
			&target.UserName, &target.CreatedAt, &target.DeletedAt, &target.Dirty, &target.Width, &target.Height,
			&target.Vote,
		); scanErr != nil {
			err = scanErr
			return
...
func (s *Store) GetPostOffset(ctx context.Context, postID int32, userLevel int32) (int32, error) {
	const q = `
SELECT COUNT(*)::int
FROM posts
WHERE id > $1
...
`
	var n int32
	err := s.Pool.QueryRow(ctx, q, postID, userLevel).Scan(&n)
	return n, err
}
